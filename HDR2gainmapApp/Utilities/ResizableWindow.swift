...
/// free above `minHeight`, force the opening size to `width × initialHeight` (overriding any restored
/// frame), and float the window above the main window. Must still be paired with
    ...
    final class Helper: NSView {
        ...
        override func viewDidMoveToWindow() {
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                // Keep the Preferences window above the main window so it stays in view while the
                // user tweaks settings and watches the preview (same level as the clipping legend).
